...
# Docsplit.extract_images('test/1.pdf', :size => '1000x', :format => [:png, :jpg], :output => 'images/')
...
CSV.open("csv/1.csv", "wb") do |csv|
	...
	e.each_line_for('images/' + input_pdf_name + '_' + image_num.to_s + '.png') do |l|
		...
		if l.text.to_s.include? "Description Quantity Units Amount"
			...
			if l.text.to_s.include? "Invoice number:" and p == 0
				r = l.text.to_s.split(':')
				...
				if a.size > 1
					...
					if a[1].include? "Invalid activity"			
						...
						unless  ( /\A[-+]?\d+\z/ === a[1])
							if a[1].include? "IO"
								w = a[1].split('IO', 2)
								q[q.size - 1][0] = q[q.size - 1][0] + 'IO' + w[1]
								q[q.size - 1][1] = q[q.size - 1][1] + w[0]
							elsif a[1].include? "SW"
								w = a[1].split('SW', 2)
								q[q.size - 1][0] = q[q.size - 1][0] + 'SW' + w[1]
								q[q.size - 1][1] = q[q.size - 1][1] + w[0]
							else
								# count = count + 1
								# puts count
